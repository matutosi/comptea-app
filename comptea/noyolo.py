...
置いてあります．必要になったら `guess_parts_v2()` を呼びます
(`guess_parts()` は 2026-09-10 の最初の版で，当たった案だけを組み直したのが v2)．

## いまの位置づけ (2026-09-14 の棚卸し)

このモジュールが受け持つのは**部分の推定** (組成部の左端・本体の上端・列の役割)
だけです．**「紙面のどこに表があるか」(表全体の箱) は `table_find.py` へ移りました**
(2026-09-11 に別に作った．タイル分割 OCR で表頭の項目名を目印にする)．
はじめは両方をここで試していたので，下の「実画像で確かめた結果」には表全体の話も
混ざっています．

- 工程 (`cli/run_pipeline.py` から辿る経路) からは **import されていません**．
  repo 全体で呼ばれるのは `table_find` が借りる `guess_pitch` の 1 つだけです．
- 表全体の検出は，**本のページなら学習済みのレイアウト検出が桁違いに強く**
  (yomitoku 73 枚・DocLayout-YOLO 74 枚 / 78 枚に対し `table_find` は 64 枚)，
  **A0 の折込は幾何の切り分けが最良** (`split_sheet.find_tables` が 21/23 枚)．
  どちらの用途でも，このモジュールを表全体に当てる出番はありません．
  なお**折込の切り分けの候補は，2026-09-14 のユーザ決定で幾何と目印の 2 つだけ**に
  なりました (レイアウト解析は折込では 1 枚も救わない．上の 73・74 枚は
  **本のページ**の話です)．
- 的は `tests/test_noyolo.py` (合成データだけ．実データは要りません)．
