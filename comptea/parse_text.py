...
# あいだにあるカンマは区切りとみなさない．
# **区切りのカンマが印字で落ちていることがある**(2026-09-08 に実物で確認．
# `Osmunda japonica ゼンマイ K—+Viola kusanoana オオタチツボスミレ K—+`)．
# 被度のあとに**属名が始まる**なら，カンマが無くてもそこで切る
# (`S—+, K—+・2` のように階層が続く形は，`K—` が小文字を伴わないので切れない)
_ENTRY_SPLIT = re.compile(
    r'(?<![\d+r十])[,，]|[,，](?!\s*\d)'
    r'|(?<=[\d+r十])\s*(?=[A-Z][a-z]{2,}\s)')
