...
def erase_box_lines(img, frac=BOX_LINE_FRAC):
    ...
    (手元の旧見本 example.jpg の 2 セル．ユーザ指摘で枠線の影響を調べた)．
...
def glyph_gate(cell, big=GLYPH_BIG, vline=GLYPH_VLINE, hline=GLYPH_HLINE,
    ...
    落ちていた (表全体でも列ごとでも．`docs/lessons_history.md`
    「2026-09-17 読み直しの高さを列ごとに測る案は，採らない」と
    「2026-09-17 罫線はセルごとに外し，読み直すかは形で決める — 採った」)．`+` と `・` は
