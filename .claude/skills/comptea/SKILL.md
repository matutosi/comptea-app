...
- 入れ方(extra の選び方と，`pip install -e .` が要る入口の別)は
  [README の「入れ方」](../../../README.md#入れ方)
...
最初に画像そのものを Read する．ここで見ておくのは3つ．
...
流し込みが次のページへあふれたとき，どのページが続きかは
**ユーザがファイル名の枝番で示す** (`xxx-1.jpg` が組成表，`xxx-2.jpg` が続き．枝番は読む順)．
規則の正は [pipeline.md の 11 節](../../../docs/pipeline.md#11-続きのページをつなぐ)．要点は次のとおり．

- **枝番が付いていれば，表が無くても異常ではない** (`run_pipeline.py` は止まらず塊を切り出す)．
  **枝番の無いページで表が無いのは異常**
- **ノンブルの順で判断しない**．枝番を正とする
- 通し終えたら `python cli/link_pages.py work --out out` で表につなぐ
...
**読み方は `--reader` で選ぶ** (`easyocr` が既定．ほかに `multi`・`ai`・`both`)．
4 つの違いは [pipeline.md の「読み方 (`--reader`)」](../../../docs/pipeline.md#読み方---reader) が正．
...
量の目安は，手元の全 147 表の要確認の件数
([README の「できること」](../../../README.md#できること)) を 24 セル/枚で割って **40 枚ほど**．1 表なら数枚に収まるので，
