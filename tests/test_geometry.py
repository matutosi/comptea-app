...
def test_隙間から等間隔の格子を組み立てる():
    ...
    """
    edges = sw.column_edges_from_gaps([100, 200, 300, 400], 0, 500)
...
def test_隙間が抜けていても間隔は保たれる():
    ...
    edges = sw.column_edges_from_gaps([100, 200, 400], 0, 500)
    assert edges is not None and len(edges) == 6


def test_隙間が少なすぎれば当てない():
    assert sw.column_edges_from_gaps([100, 200], 0, 500) is None
...
def test_字を割る回数を数える():
    ...
    cross = sw.crossing_counts(dark, [(0, 12), (12, 30)], 100)
...
def test_かたまりの外は数えない():
    ...
    cross = sw.crossing_counts(dark, [(0, 20)], 60)
